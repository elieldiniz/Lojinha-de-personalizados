import { FormaPagemento } from '@iroperson/core'

export interface SelecaoFormaPagamentoProps {
    formaPagamento?: FormaPagemento
    formaPagamentoMudou?: (value: FormaPagemento) => void
    className?: string
}
...
export default function SelecaoFormaPagamento(
    ...
    function renderizarItem(label: string, tipo: FormaPagemento) {
    ...
    return (
        <div className={`flex flex-col gap-3 ${props.className ?? ''}`}>
            ...
            <div className="flex flex-col gap-3">
                {renderizarItem('Pagamento no PIX', FormaPagemento.PIX)}
                {renderizarItem('Boleto Bancário', FormaPagemento.BOLETO)}
                {renderizarItem('Cartão de Crédito', FormaPagemento.CARTAO)}
            </div>
        </div>
    )
}
